...
SCRIPT = REPO_ROOT / "scripts" / "analyze_tlpips_curve_from_renders.py"
VENV_PY = REPO_ROOT / "third_party" / "FreeTimeGsVanilla" / ".venv" / "bin" / "python"


def _write_concat_frame(path: Path, pred_value: int, w: int = 64, h: int = 64) -> None:
...
def test_tlpips_curve_from_renders_contract(tmp_path: Path) -> None:
    ...
    out_csv = tmp_path / "tlpips_curve.csv"
    ...
    required_cols = {"pair_idx", "frame_prev", "frame_cur", "tlpips"}
