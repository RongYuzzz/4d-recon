...
class Config:
    ...
    force_zero_velocity_for_t0: bool = False
    """For T0 zero-velocity degeneration checks only.
    When enabled, all initialized velocities are hard-set to 0."""

    t0_debug_interval: int = 0
    """If >0, print T0 audit stats every N steps (t range, ||v||, ||Δx||, grad norms)."""

    t0_grad_log_path: str = ""
    """Optional CSV file path for T0 grad logging (step, ||grad_v||, ||grad_duration||, finite flags)."""
...
class FreeTime4DRunner:
    ...
    def __init__(self, local_rank: int, world_rank: int, world_size: int, cfg: Config):
        ...
        if cfg.force_zero_velocity_for_t0:
            raw_vel = init_data["velocities"]
            raw_norm = torch.linalg.vector_norm(raw_vel, dim=-1)
            print(
                "[T0] force_zero_velocity_for_t0 enabled: "
                f"zeroing velocities (before: min/mean/max="
                f"{raw_norm.min().item():.6f}/{raw_norm.mean().item():.6f}/{raw_norm.max().item():.6f})"
            )
            init_data["velocities"] = torch.zeros_like(raw_vel)
        ...
        # Optional T0 grad log
        self.t0_grad_log_path = cfg.t0_grad_log_path.strip()
        if self.t0_grad_log_path:
            grad_dir = os.path.dirname(self.t0_grad_log_path)
            if grad_dir:
                os.makedirs(grad_dir, exist_ok=True)
            if not os.path.exists(self.t0_grad_log_path):
                with open(self.t0_grad_log_path, "w", encoding="utf-8") as f:
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            if cfg.t0_debug_interval > 0 and step % cfg.t0_debug_interval == 0:
                with torch.no_grad():
                    vel_norm = self.splats["velocities"].norm(dim=-1)
                    displacement = self.splats["velocities"] * (t - self.splats["times"])
                    dx_norm = displacement.norm(dim=-1)
                    t_min = float(t_batch.min().item())
                    t_max = float(t_batch.max().item())
                    print(
                        f"[T0][Step {step}] t_range=[{t_min:.4f},{t_max:.4f}] "
                        f"|v| min/mean/max={vel_norm.min().item():.6f}/{vel_norm.mean().item():.6f}/{vel_norm.max().item():.6f} "
                        f"|dx| min/mean/max={dx_norm.min().item():.6f}/{dx_norm.mean().item():.6f}/{dx_norm.max().item():.6f}"
                    )
            ...
            vel_grad = self.splats["velocities"].grad
            dur_grad = self.splats["durations"].grad
            vel_grad_norm = float(vel_grad.norm().item()) if vel_grad is not None else float("nan")
            dur_grad_norm = float(dur_grad.norm().item()) if dur_grad is not None else float("nan")
            vel_grad_finite = int(math.isfinite(vel_grad_norm))
            dur_grad_finite = int(math.isfinite(dur_grad_norm))

            if cfg.t0_debug_interval > 0 and step % cfg.t0_debug_interval == 0:
                print(
                    f"[T0][Grad][Step {step}] "
                    f"||grad_v||={vel_grad_norm:.6e} (finite={vel_grad_finite}) "
                    f"||grad_duration||={dur_grad_norm:.6e} (finite={dur_grad_finite})"
                )

            if self.t0_grad_log_path:
                with open(self.t0_grad_log_path, "a", encoding="utf-8") as f:
                    f.write(
                        f"{step},{vel_grad_norm:.10e},{dur_grad_norm:.10e},{vel_grad_finite},{dur_grad_finite}\n"
                    )
