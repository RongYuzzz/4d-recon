# Internal Code Audit (Phase 3/4/6/7 + Pre‑Expert Seed Replication) — Why `psnr_fg↑ & lpips_fg↓` Didn’t Stably Land

Date: 2026-03-05  
Scope: **Phase 3 / Phase 4 / Phase 6 / Phase 7 / Pre‑Expert seed replication** (THUman4.0 s00, local-only eval)  
...
## 5.1) Pre‑Expert Seed Replication Confirms Seed Sensitivity (Not an Eval Bug)

We ran the cheapest replication for the only observed “FG win” weak setting:
- setting: weak `staticp99 + w0.8`
- seeds: `43`, `44` (baseline+treatment paired per seed; same-init fairness enforced)
- gate: `Δpsnr_fg > 0` and `Δlpips_fg < 0`, guardrail `ΔtLPIPS <= +0.01`

Result (step=599, treat - base; see `notes/openproposal_preexpert_seedrep_staticp99.md`):
...
So `OVERALL_OK=False` under the strict gate. This indicates the positive signal is **seed-sensitive**, not explained by confounds or broken eval.
